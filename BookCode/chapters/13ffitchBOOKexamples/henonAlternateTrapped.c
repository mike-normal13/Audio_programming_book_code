...
int squence[] = { 1, 1, 1, 1, 1, 2, 2, 1, 1, 1, 1, 4, 2, 2, 2, 
                  4, 3, 1, 1, 1, 1, 5, 5, 6, 9, 9, 9, 9, 9, 9, 
                  3, 6, 7, 7, 7, 3, 7, 8, 8, 8, 7, 7, 9, 9, 9, 
                  9, 9, 9, 9, 9, 2, 2, 2, 2, 2, 2, 2, 4, 4, 9,
                  9, 9, 9, 11, 13, 9, 9, 2, 11, 2, 8, 11, 9,
                  9, 9, 9, 11, 11, 13, 13, 9, 9, 9, 2, 8, 2, 
                  11, 8, 2, 2, 11, 11, 11, 11, 4, 4, 4, 5, 5, 
                  5, 5, 4, 4, 4, 4, 5, 5, 4, 4, 4, 4, 12, 12, 
                  12, 4, 4, 4, 12, 10, 10, 12, 10, 4, 4, 4, 4,
                  4, 4, 4, 4, 12, 10, 10, 10, 10, 10, 10, 10, 
                  10, 10, 10, 10, 10, 10, 10, 8, 8, 8, 8, 8,
                  8, 8, 4, 10, 10, 10, 10, 2, 2, 4, 4, 10, 10,
                  10, 10, 10, 10, 10 }; 

int selectinstr(double tt)
{
    int which;
    double ch = 168.0*tt/323.0;
    which = (int)(ch+0.5);
    if (which>167) return 1;  /* Force last event */
    if (which== 0) return 0;  /* and start event */
...
void instr_1(double tt, double dur, double pitch, 
                        double xx, double yy)
{
/* p6=amp,p7=vibrat,p8=glisdeltime(default < 1),p9=frqdrop */
    ...
    frqdrop = 0.99;
              /* Should be a table */
    pitch   = 440.0*pow(2.0,((int)pitch-40.0)/12.0);
    printf("i1 %f %f 0 %f %f %.3f %f %.2f\n",
           tt, 3.0*dur, pitch, amp, vibrat, glis, frqdrop);
}

void instr_2(double tt, double dur, double pitch,
                        double xx, double yy)
{
/* p6=amp,p7=rvbsnd,p8=lfofrq,p9=#harmonics,p10=sweeprate */
    ...
    while (dur>20.0)
         dur -= 10.0;
    pitch     = 440.0*pow(2.0,((int)pitch-34.0)/12.0);
    printf("i2 %f %f 0 %f %f %f %f %f %f\n",
           tt, 81.0*dur, pitch, amp, rvbsnd, lfofrq,
           numh, sweeprate);
}
