...
main(int argc, char **argv)
{
	...
	OSCMOD *oscs;
	MIXOUT *mixes;
	NOISEMOD *noises; /* We now define a pointer for the noise modules here. */
	
	int osc_count = 0;
	int mix_count = 0;
	int noise_count = 0; /* We now keep count of noise modules. */
	char modname[64];
	int i;
	float duration = 10.0; 
	...
	/* Here is our new allocation call */
	...
	while(scanf("%s", modname) != EOF){
		...
		/* Notice that we put noise before the MIXOUT. Otherwise
		the noise would not be mixed out. */
	...
	/* Our call to initialize_globals now initializes noise too. */
	initialize_globals(oscs, osc_count, noises, noise_count);
	
	for(i = 0; i < osc_count; i++){
		print_osc(oscs[i]);
	}
	/* We print the noise here: */
